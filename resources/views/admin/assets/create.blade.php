    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.assets.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('price') ? 'is-invalid' : '' }}" type="number" name="price" id="price" step="0.01" value="{{ old('price', '') }}">
                @if($errors->has('price'))
                    <div class="invalid-feedback">
                        {{ $errors->first('price') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control date {{ $errors->has('price') ? 'is-invalid' : '' }}" type="text" name="warranty_expiry_date" id="warranty_expiry_date" value="{{ old('warranty_expiry_date', '') }}">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('parent_id') ? 'is-invalid' : '' }}" id="group_id" name="group_id">
                    @foreach($parentGroups as $id => $parentGroup)
                        <option value="{{ $id }}" {{ old('group_id', '') == $id ? 'selected' : '' }}>{{ $parentGroup }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('sub_group_id') ? 'is-invalid' : '' }}" id="sub_group_id" name="sub_group_id" required>
                    @if(old('sub_group_id', '') && isset($subGroups[old('group_id', '')]))
                        @foreach($subGroups[old('group_id', '')] as $id => $group)
                            <option value="{{ $id }}" {{ old('sub_group_id', '') == $id ? 'selected' : '' }}>{{ $group }}</option>
                        @endforeach
                    @endif
                </select>
...
<script>
    ...
    $(function() {
        ...
        @if (!old('group_id', '') || !old('sub_group_id', ''))
            changeSubGroups()
        @endif
