    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.images.update", $image) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <textarea class="form-control {{ $errors->has('description') ? 'is-invalid' : '' }}" name="description" id="description">{{ old('description', $image->description) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('asset_id') ? 'is-invalid' : '' }}" id="asset_id" name="asset_id">
                        @foreach($assets as $id => $asset)
                            <option value="{{ $id }}" {{ old('asset_id', $image->asset_id) == $id ? 'selected' : '' }}>{{ $asset }}</option>
                        @endforeach
                    </select>
